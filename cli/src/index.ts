    ...
    .command(
      ...
      (y) =>
        y
          ...
          .option('lockfile', {
            type: 'string',
            choices: ['check', 'refresh', 'enforce'],
            describe:
              'Lockfile handling mode:\n' +
              '  check   - Validate existing lockfile: ensure it exists, is syntactically correct,\n' +
              '            matches package.json, and all locked versions are resolvable (fails if invalid)\n' +
              '  refresh - Refresh/create lockfile with latest resolved versions\n' +
              '  enforce - Validate if lockfile exists, create if missing',
          })
          .option('concurrency', {
            type: 'number',
            default: 10,
      ...
      async (args) => {
        ...
        try {
          const result = await scanPath(inputPath, {
            includeDev: args.dev,
            lockfileMode: args.lockfile as 'check' | 'refresh' | 'enforce' | undefined,
